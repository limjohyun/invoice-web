...
- **문서 기준일**: 2026-07-02
- **현재 상태**: 스타터 템플릿 기반 완료(디자인 시스템, 폼 표준, 홈/로그인/회원가입 페이지 마크업, ESLint/Prettier/Husky). 실제 인증 백엔드, Notion 연동, 견적서 기능은 **전무한 상태**(package.json에 `@notionhq/client`, Supabase, PDF 라이브러리 미설치 확인)
...
> 📌 **핵심 설계 이슈(착수 전 반드시 결정)**: PRD의 데이터 모델은 User(계정)·Client(거래처)·Invoice(견적서)·Template(템플릿) 4종이지만, F011은 "연동할 데이터베이스"를 단수로 표현합니다. Notion에는 계층형 데이터베이스가 없으므로 **거래처/견적서/템플릿을 별도의 Notion 데이터베이스 3개로 분리하고, User는 Supabase에 저장(비밀번호+토큰+3개 DB ID 매핑)**하는 구조를 기본안으로 제안합니다. 이 결정은 Phase 1 착수 즉시 확정해야 이후 모든 Notion 연동 작업의 기준이 됩니다.
...
#### 핵심 기능
- [ ] (MUST) Supabase 프로젝트 생성 및 `@supabase/supabase-js`, `@supabase/ssr` 설치·환경 변수(`src/lib/env.ts`) 확장
- [ ] (MUST) F010 회원가입/로그인/로그아웃 — 기존 `login-form.tsx`/`signup-form.tsx`를 Server Action + Supabase Auth로 연결
- [ ] (MUST) `middleware.ts` 기반 보호된 라우트(로그인 필요 페이지 접근 제어)
- [ ] (MUST) F011 Notion 연동 설정 페이지 — Integration Token 입력/저장, 연동 가능 DB 목록 조회, 연결 테스트 UI
- [ ] (MUST) 로그인 성공 후 분기 로직: Notion 미연동 → 연동 설정 페이지, 연동 완료 → 대시보드(PRD 사용자 여정 그대로)

#### 기술적 준비 작업
- [ ] Notion 데이터베이스 구조 확정(설계 스파이크, 0.5일): 거래처/견적서/템플릿 DB 스키마 초안 + Notion 웹에서 실제 DB 3개 생성해 PoC
- [ ] `@notionhq/client` 설치 및 서버 전용 Notion API 래퍼 모듈 설계(`src/lib/notion/client.ts`)
- [ ] Notion Integration Token 저장 방식 결정(Supabase 테이블 컬럼 암호화 저장 — 평문 저장 금지)
- [ ] Supabase 세션 타입과 Server Action 간 타입 공유 패턴 정리(`src/lib/schemas/auth.ts` 확장)

#### 예상 완료 결과물
- 실제 이메일/비밀번호로 가입·로그인·로그아웃이 동작하는 인증 시스템
- Notion Integration Token을 입력하고 연동 테스트에 성공할 수 있는 설정 페이지
- 확정된 Notion DB 스키마 문서(내부 설계 메모)

#### 위험 요소
- **위험**: Notion Integration Token을 평문으로 저장하면 보안 사고로 직결 → **완화**: Supabase 컬럼 레벨 암호화 또는 Vault 기능 사용, 최소한 서버 전용 접근(RLS로 클라이언트 직접 조회 차단)
- **위험**: 거래처/견적서/템플릿을 DB 3개로 분리하는 설계가 이후 관계(견적서→거래처 relation) 설정에서 Notion Relation 속성 제약에 부딪힐 수 있음 → **완화**: Phase 1 설계 스파이크에서 실제 Notion Relation 속성으로 PoC 먼저 검증 후 Phase 2 착수
...
| 마일스톤 | 예상 완료일 | 핵심 산출물 |
|---------|-----------|----------|
| M1: 인증 & Notion 연동 기반 | 2026-07-15 | 실동작 로그인/회원가입, Notion Integration 연동 설정 페이지, DB 스키마 확정 |
| M2: 견적서 핵심 CRUD | 2026-07-29 | 대시보드, 견적서 생성/조회/수정/삭제(Notion 실데이터) |
| M3: **MVP 출시** | 2026-08-08 | F001~F013 전 기능 완성, PDF 다운로드, 거래처/템플릿 관리 |
| M4: 안정화 | 2026-08-22 | 스모크 테스트, 에러 처리, 로딩 UX 개선 |
| M5: 배포 & 문서화 완료 | 2026-09-04 | 프로덕션 배포, Notion 연동 문서, 성능 점검 완료 |
