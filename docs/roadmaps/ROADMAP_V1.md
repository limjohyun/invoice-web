...
### Phase 6: 관리자 기능 추가 (2026-07-20 완료 · 원 계획 2026-09-07 ~ 2026-09-25)
...
- [x] (MUST) F014 관리자 뷰 — 기존 `/invoice` 목록 페이지를 확장해 각 견적서 행에 "공유 링크 복사" 액션 추가(신규 `/admin` 라우트 신설 없음, 쟁점 1 결정 근거 참고) **✅ 구현+Chrome 검증 완료**
- [x] (MUST) F015 클라이언트 공유 링크 — 견적서별 랜덤 공유 토큰 발급/조회, 공개 열람 페이지(`/share/[token]`)에서 비로그인 상태로 견적서 상세(거래처 정보, 품목, 합계) 열람 가능 **✅ 구현+Chrome 검증 완료**
- [x] (SHOULD) 공개 페이지에서 PDF 다운로드(`/api/share/[token]/pdf`) — 로그인 없이 클라이언트가 직접 PDF 저장 가능 **✅ 구현+검증 완료(curl로 200/application/pdf 확인)**
- [x] (SHOULD) 공유 링크 비활성화(회수) — 소유자가 관리자 뷰에서 링크를 무효화하면 이후 접근 시 "만료된 링크" 안내 **✅ 구현+Chrome 검증 완료**
- [x] (MUST로 승격·완료) 견적서 삭제 시 연결된 공유 링크 자동 무효화 — `deleteInvoice`에서 `deactivateShareLinksForInvoice` 호출 **✅ 구현+검증 완료**(더미 견적서 생성→링크 발급→실제 삭제→DB `is_active: false` 전환→공개 페이지 "유효하지 않은 링크" 확인)
- [ ] (NICE-TO-HAVE) 링크 만료일 설정(발급 시 30/90일/무제한 선택) 및 재발급(기존 토큰 폐기 후 신규 발급) — 미구현
- [ ] (NICE-TO-HAVE) 견적서 상태 변경 시 연결된 공유 링크 자동 무효화 — 미구현(삭제 시 무효화만 구현됨)

#### 기술적 준비 작업

- [x] Supabase 마이그레이션: `invoice_share_links` 테이블 신설(`id`, `invoice_id`, `owner_user_id`, `token`(unique, 32자 이상 랜덤), `is_active`, `expires_at`(nullable), `created_at`) + RLS 정책(소유자 본인만 자신의 링크 CRUD 가능, Service Role은 정책과 무관하게 우회) — `supabase/sql/invoice_share_links.sql` **✅ 사용자가 Supabase 대시보드에서 직접 적용 완료**
- [x] 공유 토큰 생성 유틸 및 충돌 시 재시도 로직 **✅ 완료** — `node:crypto`의 `randomBytes(32).toString('base64url')`(43자) 사용, `token` 컬럼 unique 제약으로 충돌 자체가 사실상 불가능해 별도 재시도 로직은 불필요하다고 판단
- [x] `src/lib/actions/share.ts` 신설: `createOrGetShareLink(invoiceId)`(기존 활성 토큰 있으면 재사용, 없으면 발급), `revokeShareLink(invoiceId)`, `getInvoiceByShareToken(token)`(Service Role로 토큰→소유자→Notion 컨텍스트 조회 후 `queryItemsByInvoice`/`retrieveInvoice` 재사용, `getNotionContext()`는 사용하지 않음) **✅ 완료**
- [x] `middleware.ts`의 `PROTECTED_PATHS`에 `/share`를 절대 추가하지 않도록 명시적 회귀 테스트 항목으로 남김(공개 라우트가 실수로 보호되면 기능 자체가 무의미해짐) **✅ 확인 완료**(curl로 비인증 상태 200/404 응답, 로그인 리다이렉트 없음)
- [x] 공개 페이지(`/share/[token]/page.tsx`)에 `<meta name="robots" content="noindex">` 적용 — 검색엔진에 견적서 링크가 노출되지 않도록 **✅ 완료**
- [x] 공개 라우트(`/share/[token]`, `/api/share/[token]/pdf`)에 대한 간단한 요청 빈도 제한 검토(토큰 무차별 대입 방어) **✅ 검토 완료** — 계획대로 정교한 rate limiting 인프라는 과설계로 판단해 도입하지 않고, 32바이트(43자) 토큰 엔트로피로 1차 방어. 정교한 rate limit은 실사용 트래픽 확보 후 후속 검토로 유지

#### 🧪 테스트 체크리스트 (Playwright MCP 필수)

**구현 완료 후 반드시 다음 3가지 시나리오를 테스트한 후에만 완료 처리**:

- [x] ✅ 정상 케이스: 관리자 뷰(`/invoice`)에서 링크 복사 클릭 → 클립보드에 유효한 `/share/[token]` URL 복사 확인 → 시크릿 창(비로그인 상태)에서 해당 URL 접속 시 견적서 상세가 정상 표시 → PDF 다운로드까지 성공 **완료** — Chrome으로 실제 로그인 세션에서 링크 복사(DB에 토큰 생성 확인) → 새 탭(비인증 컨텍스트와 동일하게 세션 무관 공개 라우트)에서 접속해 거래처정보/품목/합계 정상 렌더링, `curl`로 PDF 200/`application/pdf`/10KB 확인
- [x] ❌ 실패/에러 케이스: 존재하지 않는 토큰, 회수(비활성화)된 토큰, 만료된 토큰으로 접근 시 "만료된 링크" 안내 페이지 표시(500 에러로 새지 않는지 확인), Notion API 실패 시 에러 메시지 **완료** — 세 가지 토큰 상태 모두 200(페이지)/404(PDF)로 친절한 안내만 표시됨을 확인. 검증 중 **버그 1건 발견·수정**: Notion 조회가 `not_found`가 아닌 다른 에러(예: 잘못된 페이지 ID 형식)로 실패하면 PDF 라우트가 500과 함께 Notion 원본 에러 메시지를 그대로 노출하던 문제 — 공개 라우트는 실패 원인과 무관하게 항상 "유효하지 않은 링크"로만 응답하도록 `getInvoiceByShareToken` 수정(커밋 `7f7d2de`)
- [x] 🎯 엣지 케이스: 견적서 삭제 후 기존 공유 링크 접근, 같은 견적서에 대해 링크를 여러 번 복사했을 때 토큰이 중복 발급되지 않는지, `/share` 및 `/api/share/*` 경로가 `middleware.ts`의 `PROTECTED_PATHS`에 걸려 로그인 리다이렉트되지 않는지 회귀 확인 **완료** — 더미 견적서로 삭제 cascade 실증 확인(DB `is_active: false` 전환 + 공개 페이지 무효화 반영), `createOrGetShareLink` 3연속 호출 시 토큰 재사용·활성 행 1개 유지 확인, 미들웨어 회귀 없음(비인증 200/404, 리다이렉트 없음)
- [x] 테스트 실패 시 원인 파악 → 코드 수정 → 재테스트 → 통과 확인 후 완료 처리 **완료** — 위 500 노출 버그를 고치고 재테스트까지 통과 확인
...
| M6: 관리자 기능 & 공유 링크    | **✅ 2026-07-20** | **완료**                  | ✅ 설계 결정(role 미도입, 토큰 기반 공유 링크), ✅ 관리자 뷰 링크 복사/비활성화 UI, ✅ 공개 열람 페이지(`/share/[token]`), ✅ 공개 PDF 다운로드, ✅ 견적서 삭제 시 링크 자동 무효화, ✅ 실패·엣지 케이스 검증(버그 1건 발견·수정). ⏳ 링크 만료일 설정·상태변경 시 자동 무효화(NICE-TO-HAVE)만 후속 과제 |
...
7. **관리자 기능 완성도(Phase 6)**: role 기반 권한 체계 없이도 관리자 뷰(기존 `/invoice` 확장)에서 공유 링크를 발급할 수 있고, 비로그인 클라이언트가 해당 링크로 견적서 열람 및 PDF 다운로드까지 중단 없이 성공하며, 소유자가 링크를 즉시 비활성화할 수 있음을 실제 시나리오로 확인 **✅ 달성(2026-07-20)**
8. **공개 라우트 안전성**: `/share/[token]`, `/api/share/[token]/pdf`가 `middleware.ts`의 `PROTECTED_PATHS`에 걸리지 않으면서도, 유효하지 않은/비활성화된 토큰으로는 어떤 견적서 데이터도 노출되지 않음을 회귀 테스트로 확인 **✅ 달성(2026-07-20)** — 검증 중 발견한 500 에러 노출 버그도 수정 완료
