...
#### 🧪 **테스트 필수 원칙 (API 연동 / 비즈니스 로직 작업)**

**구현 직후 테스트는 선택이 아닌 필수 게이트입니다.** API 연동, 인증, 데이터베이스 조작, 계산 로직 등 비즈니스 로직을 포함하는 모든 Task는 구현 즉시 Playwright MCP 테스트를 수행해야 하며, 테스트 통과 없이는 다음 단계로 진행할 수 없습니다.

**테스트 시나리오 3가지 필수 커버** (최소 요구사항):
1. **정상 케이스(Happy Path)**: 올바른 입력값으로 기대한 결과 획득 검증
2. **실패/에러 케이스**: 잘못된 입력, API 실패, 네트워크 오류, 권한 부족 등 예외 상황 처리 확인
3. **엣지 케이스**: 경계값(최소/최대), 빈 값, 중복 요청, 동시성 이슈 등 특수 상황 처리 확인

**테스트 실패 시 처리 규칙**:
- ❌ 테스트 실패 시 작업을 완료(✅) 상태로 표시하지 않습니다.
- 🔧 원인을 파악하고 구현 코드를 수정합니다.
- 🔄 수정된 코드에 대해 즉시 재테스트를 수행합니다.
- ✅ 테스트가 통과한 후에만 작업을 완료 처리합니다.

**각 작업 파일 기록**:
- 작업 파일의 `## 테스트 체크리스트` 섹션에 실행한 각 시나리오와 결과(✅ 통과 / ❌ 실패)를 명시합니다.
- 테스트 실패 후 재테스트한 경우, 수정 내용과 재테스트 결과도 함께 기록합니다.
...
- **API/비즈니스 로직 작업 시 "## 테스트 체크리스트" 섹션 필수 포함**: 정상/실패/엣지 케이스별로 구체적인 Playwright MCP 테스트 시나리오를 작성해야 하며, **테스트 통과 없이는 작업을 완료 처리하지 않습니다.** 테스트 실패 시 수정 후 재테스트를 수행한 결과도 함께 기록합니다.
...
- 작업 파일의 명세서를 따름
- 기능과 기능성 구현
- **API 연동 및 비즈니스 로직 구현 시 Playwright MCP로 정상/실패/엣지 케이스 테스트를 모두 수행 필수**
- 각 단계 후 작업 파일 내 단계 진행 상황 업데이트
- 구현 완료 후 Playwright MCP를 사용한 E2E 테스트 실행
- **테스트 실패 시**: 완료 상태로 표시하지 말고, 원인을 파악하고 코드를 수정한 후 즉시 재테스트를 수행합니다. 테스트 통과 확인 후에만 완료 처리합니다.
...
- **Task 005: 데이터베이스 및 API 개발** - 우선순위
  ...
  - Playwright MCP 테스트 (정상 케이스: 올바른 요청에 대한 응답 검증)
  - Playwright MCP 테스트 (실패 케이스: 잘못된 입력, DB 오류, 네트워크 실패 처리)
  - Playwright MCP 테스트 (엣지 케이스: 경계값, 동시 요청, 중복 처리)
  - 테스트 통과 후에만 다음 단계로 진행

- **Task 006: 인증 및 권한 시스템 구현**
  - 사용자 인증 시스템 구축
  - 권한 기반 접근 제어 구현
  - 보안 미들웨어 및 세션 관리
  - Playwright MCP 테스트 (정상 케이스: 유효한 인증정보로 로그인 성공)
  - Playwright MCP 테스트 (실패 케이스: 잘못된 비밀번호, 존재하지 않는 사용자, 토큰 만료)
  - Playwright MCP 테스트 (엣지 케이스: 권한 없는 페이지 접근, 세션 동시성)
  - 테스트 통과 후에만 다음 단계로 진행

- **Task 006-1: 핵심 기능 통합 테스트**
  - Playwright MCP 테스트 (정상 케이스: 전체 사용자 플로우 성공 시나리오)
  - Playwright MCP 테스트 (실패 케이스: 부분 실패 시 에러 메시지 및 롤백)
  - Playwright MCP 테스트 (엣지 케이스: 네트워크 불안정, 동시 작업, 데이터 일관성)
  - 모든 테스트 통과 확인 후 완료 처리
...
- [ ] 구현 완료 후 테스트를 건너뛰지 않고 반드시 수행했는가?
- [ ] 테스트 시나리오가 정상/실패/엣지 케이스를 모두 포괄하는가?
- [ ] 테스트 실패 시 완료 처리 전 수정·재테스트가 이루어지는 흐름으로 작성되었는가?
